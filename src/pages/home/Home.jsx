...
const Home = () => (
  <>
    <h1 className={css.title}><span>Розширювана колекція React компонентів</span></h1>
    ...
    <ul className={css.dependencies}>
      {DEPENDENCIES.map(({ id, name, link }) => (
        <li className={css.dependencyItem} key={id}>
          <a className={css.dependencyLink} href={link} rel="noopener noreferrer" target="_blank">{name}</a>
        </li>
      ))}
    </ul>    
  </>
